...
%These different aspects of stability are briefly presented in  \hyperref[stability_others]{appendix}.
...
\newpage
\addcontentsline{toc}{section}{Personal reviews}
...
\paragraph{}
Another way to estimate this probability is by using the estimation of $N^*$ and $W^*$ and deducing the risk to go below the Allee threshold effect (which is $A$). %\todo{} 
% Here we consider that the severity s(N^*+\alpha W^*) follow an exponential distribution
% np.exp(-1./strength*(N_star-a)/(N_star+alpha*W_star))
\[
\begin{array}{rcl}
